...
PRIV_TIMER_CHECK:
   			//... [FILL IN CODE HERE ]
CMP     R5, #MPCORE_PRIV_TIMER_IRQ  // compare store ID with the timer to see if the timer caused the interrupt
BNE     KEYS_CHECK  //if it's not the timer, then check the keys
BL      PRIV_TIMER_ISR  //if it's the timer, proceed to Priv_Timer which implements the pattern and rotations
B       EXIT_IRQ
    
KEYS_CHECK:
    			//... [FILL IN CODE HERE ]
CMP     R5, #KEYS_IRQ
BL      KEY_ISR // if key caused interrupt, go to KEY_ISR and handle it
